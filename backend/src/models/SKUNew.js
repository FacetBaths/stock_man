...
skuNewSchema.pre('save', function(next) {
...
skuNewSchema.methods.addCost = function(cost, updatedBy, notes = '') {
...
skuNewSchema.methods.getCostAtDate = function(date) {
...
skuNewSchema.methods.getStockStatus = function(currentQuantity) {
...
skuNewSchema.methods.getDisplayName = function() {
...
skuNewSchema.index({ sku_code: 1 });
skuNewSchema.index({ category_id: 1, status: 1 });
skuNewSchema.index({ barcode: 1 });
skuNewSchema.index({ status: 1 });
skuNewSchema.index({ name: 1 });
skuNewSchema.index({ brand: 1 });
skuNewSchema.index({ created_by: 1 });
skuNewSchema.index({ 'details.tool_type': 1 }); // For tool filtering

module.exports = mongoose.model('SKUNew', skuNewSchema);
