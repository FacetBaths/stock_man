...
const returnCheckout = async () => {
  ...
  try {
    ...
    console.log('🔄 Returning all tools from checkout:', selectedLoanForAction.value._id)
    const response = await toolsApi.returnCheckout(selectedLoanForAction.value._id, returnReason.value)
    ...
    console.log(`✅ All tools returned to inventory and checkout completed - dashboard refreshed`)
...
const loadAvailableTools = async (search = '') => {
  try {
    ...
    const tools = response.inventory?.map(inv => ({
    ...
    console.log('🔍 Transformed tools with real-time inventory:', tools.map(tool => ({
      name: tool.name,
    ...
    checkoutError.value = 'Failed to load available tools: ' + (err.message || 'Unknown error')
  } finally {
...
const filteredTools = computed(() => {
  ...
  if (toolCategoryFilter.value !== 'all') {
    ...
    filtered = filtered.filter(tool => 
      ...
      tool.category_id?.name?.toLowerCase() === toolCategoryFilter.value.toLowerCase()
    )
    console.log(`🔍 After category filter (${toolCategoryFilter.value}): ${beforeCategoryFilter} -> ${filtered.length}`)
  }
  ...
  if (toolSearchQuery.value.trim()) {
    ...
    filtered = filtered.filter(tool => {
      ...
      return toolName.includes(query) || toolCode.includes(query) || category.includes(query)
    })
    console.log(`🔍 After search filter (${query}): ${beforeSearchFilter} -> ${filtered.length}`)
  }
  
  // Debug: Show inventory info before filtering
  console.log('🔍 Tools with inventory info:', filtered.map(tool => ({
    name: tool.name,
    sku_code: tool.sku_code,
  ...
  filtered = filtered.filter(tool => (
    tool.inventory?.available_quantity && tool.inventory.available_quantity > 0
  ))
  ...
  console.log(`🔍 After inventory filter (DISABLED): ${beforeInventoryFilter} -> ${filtered.length}`)
...
const handleToolInput = async () => {
  ...
  try {
    ...
    const existingIndex = selectedTools.value.findIndex(selected => 
      selected.tool.sku_code === toolCode
    )
    ...
    const matchingTool = availableTools.value.find(tool => 
...
const addToolFromSelector = (tool: any) => {
  const existingIndex = selectedTools.value.findIndex(selected => selected.tool._id === tool._id)
...
const updateToolQuantity = (index: number, quantity: number) => {
  if (quantity <= 0) {
    ...
    selectedTool.quantity = Math.min(quantity, selectedTool.available_instances)
  }
}
...
const canSubmitCheckout = computed(() => {
  return checkoutForm.value.customer_name.trim() !== '' && 
...
const submitCheckout = async () => {
  try {
    ...
    console.log(`✅ Successfully checked out tools to ${checkoutData.customer_name}`)
    
  } catch (err: any) {
    console.error('❌ Failed to create tool checkout:', err)
    checkoutError.value = err?.response?.data?.message || err?.message || 'Failed to check out tools'
  } finally {
...
<template>
  <q-page class="q-pa-lg">
    <div class="row q-col-gutter-lg">
          ...
          <div class="row items-center q-gutter-md">
            ...
            <div>
              ...
              <p class="text-body1 q-ma-none text-grey-7">
                Track tool inventory, manage check-outs, and monitor availability
              </p>
            </div>
          </div>
        </div>
          ...
          <q-tab-panel name="loans" class="q-pa-none">
            <div class="glass-card q-pa-lg">
              <div class="row items-center q-mb-lg">
                ...
                <div>
                  <h5 class="text-h5 q-ma-none text-weight-bold">Tool Check-outs Management</h5>
                  <p class="text-body2 q-ma-none text-grey-7">
                    Track active check-outs, create new check-outs, and process returns
                  </p>
                </div>
                <q-space />
                <q-input
              ...
              <div v-if="isLoadingLoans" class="text-center q-py-xl">
                <q-circular-progress
                  ...
                  class="q-mb-md"
                />
                <div class="text-body1 text-grey-7">Loading active check-outs...</div>
              </div>
              ...
              <div v-else-if="filteredLoans.length === 0" class="text-center q-py-xl">
                ...
                <div class="text-h6 text-grey-6 q-mb-sm">
                  {{ activeLoans.length === 0 ? 'No Active Check-outs' : 'No Matching Check-outs' }}
                </div>
      ...
      <!-- Tool Checkout Modal -->
      <div v-if="showCreateLoanModal" class="modal-overlay tool-checkout-modal" @click.self="cancelCheckout">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-header">
              ...
              <button class="close-button" @click="cancelCheckout">&times;</button>
            </div>
            ...
            <div class="progress-container">
              <div class="progress-bar">
                <div 
                  class="progress-fill" 
                  :style="{ width: `${(checkoutStep / totalCheckoutSteps) * 100}%` }"
                ></div>
              </div>
              <div class="step-labels">
                <span :class="{ active: checkoutStep >= 1, completed: checkoutStep > 1 }">Checkout Details</span>
                <span :class="{ active: checkoutStep >= 2, completed: checkoutStep > 2 }">Select Tools</span>
                <span :class="{ active: checkoutStep >= 3 }">Review & Submit</span>
              </div>
            </div>
            ...
            <div class="modal-body">
              ...
              <div v-if="checkoutError" class="alert alert-danger">
                {{ checkoutError }}
                <button type="button" class="btn-close" @click="checkoutError = null">&times;</button>
              </div>
              ...
              <div v-if="checkoutStep === 1" class="step-content">
                ...
                <div class="form-group">
                  <label for="customer-name" class="form-label">Installer Name *</label>
                  <input
                    id="customer-name"
                ...
                <!-- Due Date -->
                <div class="form-group">
                  <label for="due-date" class="form-label">Expected Return Date (Optional)</label>
                  <input
                    id="due-date"
              ...
              <!-- Step 2: Select Tools -->
              <div v-if="checkoutStep === 2" class="step-content step-2-content">
                <h4>Select Tools to Check Out</h4>
                <p class="step-description">
                  Scan tool codes or browse & select tools to check out to <strong>{{ checkoutForm.customer_name }}</strong>.
                </p>
                ...
                <div class="two-panel-layout">
                  ...
                  <div class="sku-browser-panel">
                    ...
                    <div v-if="showToolSelector" class="panel-content">
                      ...
                      <div v-else class="skus-grid">
                        ...
                        >
                          <div class="sku-info">
                            ...
                            <div class="sku-meta">
                              <span class="sku-location">{{ tool.category_id?.name || 'Unknown Category' }}</span>
                              <span class="sku-quantity">Available: {{ tool.inventory?.available_quantity || 0 }}</span>
                              <span v-if="tool.inventory?.loaned_quantity > 0" class="reserved-quantity">
                      ...
                      <div v-if="!isLoadingTools && filteredTools.length === 0" class="no-results">
                  ...
                  <div class="selected-skus-panel">
                    ...
                    <div class="panel-content">
                      <div v-if="selectedTools.length > 0" class="selected-skus-list">
                        <div 
                        ...
                        >
                          <div class="sku-info">
                            <strong>{{ getToolDisplayName(selectedTool.tool) }}</strong>
                            <div class="sku-meta">
                              <span class="sku-code">{{ selectedTool.tool.sku_code }}</span>
                              <span class="availability">Available: {{ selectedTool.available_instances }}</span>
                            </div>
                          </div>
                          ...
                          <div class="sku-controls">
                            ...
                            <div class="quantity-controls">
                              <button 
                                type="button" 
                                class="qty-btn"
                                @click="updateToolQuantity(index, selectedTool.quantity - 1)"
                              >
                              ...
                              <input 
                                ...
                                min="1"
                                @input="updateToolQuantity(index, parseInt(($event.target as HTMLInputElement).value))"
                              />
                              <button 
                                type="button" 
                                class="qty-btn"
                                @click="updateToolQuantity(index, selectedTool.quantity + 1)"
                                :disabled="selectedTool.quantity >= selectedTool.available_instances"
                              >
                      ...
                      <div v-else class="no-selection-message">
                        ...
                        <small>Scan tool codes or browse tools to get started.</small>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="tag-summary">
                  <div class="summary-header">
                    ...
                    <div class="summary-stats">
                      <span><strong>{{ totalSelectedTools }}</strong> tools</span>
                      <span><strong>{{ totalSelectedQuantity }}</strong> total instances</span>
                    </div>
                  </div>
                    ...
                    >
                      <div class="sku-info">
                        ...
                        <div class="sku-details">
                          ...
                          <span>Category: {{ selectedTool.tool.category_id?.name || 'Unknown' }}</span>
                        </div>
                      </div>
                      <div class="sku-quantity">
                        <span class="quantity-badge">{{ selectedTool.quantity }} tools</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <ReturnToolsDialog 
        ...
        @checked-in="async () => { selectedReturnLoanId = null; await refreshDashboard(); }"
      />
          ...
          <q-card-section v-if="selectedLoan">
            ...
            <div class="q-mb-lg">
              <div class="row q-col-gutter-md q-mb-md">
                <div class="col-6">
                  <div class="text-caption text-grey-7 q-mb-xs">Customer/Installer</div>
                  <div class="text-h6 text-weight-medium">{{ selectedLoan.customer_name }}</div>
                </div>
                <div class="col-6">
              ...
              <div class="row q-col-gutter-md q-mb-md" v-if="selectedLoan.project_name">
                <div class="col-6">
                  <div class="text-caption text-grey-7 q-mb-xs">Project Name</div>
              ...
              <div class="row q-col-gutter-md">
                <div class="col-6">
                  <div class="text-caption text-grey-7 q-mb-xs">Check-out Date</div>
                  <div class="text-body2">{{ formatDateTime(selectedLoan.createdAt) }}</div>
                </div>
                <div class="col-6" v-if="selectedLoan.due_date">
                  <div class="text-caption text-grey-7 q-mb-xs">Expected Return</div>
                  <div class="text-body2" :class="{ 'text-negative': selectedLoan.is_overdue }">
            ...
            <div v-if="selectedLoan.notes" class="q-mb-lg">
              ...
              <div class="text-body2 q-pa-md" style="background: #f5f5f5; border-radius: 4px;">
                {{ selectedLoan.notes }}
              </div>
            </div>

            <!-- Tools Section -->
            <div v-if="selectedLoan.sku_items && selectedLoan.sku_items.length > 0">
              ...
              <div class="q-mb-md">
                <div class="row text-caption text-grey-7 q-pa-sm" style="background: #f8f9fa; font-weight: 500;">
                ...
                >
                  <div class="col-5">
                    ...
                    <div v-if="typeof item.sku_id === 'object' && item.sku_id.category_id" class="text-caption text-grey-6">
                      {{ item.sku_id.category_id.name }}
                    </div>
                  </div>
                  <div class="col-3">
                    <code class="text-primary text-weight-medium">
                      {{ typeof item.sku_id === 'object' ? item.sku_id.sku_code : 'N/A' }}
                    </code>
                  </div>
                  <div class="col-2">
              ...
              <div class="row q-col-gutter-md text-center">
                ...
                <div class="col">
                  <div class="text-h6 text-info text-weight-bold">
                    {{ selectedLoan.sku_items?.reduce((sum, item) => sum + (item.selected_instance_ids?.length || 0), 0) || 0 }}
                  </div>
      ...
      <q-dialog v-model="showReturnDialog" persistent>
        <q-card style="min-width: 400px;">
          <q-card-section class="row items-center">
            <q-avatar icon="assignment_return" color="warning" text-color="white" />
            <span class="q-ml-sm text-h6">Return All Tools</span>
          </q-card-section>

          <q-card-section v-if="selectedLoanForAction">
            <p class="q-mb-md">
              Are you sure you want to <strong>return all tools</strong> from this checkout?
            </p>
            <p class="q-mb-md">
              This action will:
            </p>
            ...
            <p class="text-weight-medium">
              Customer: <span class="text-primary">{{ selectedLoanForAction.customer_name }}</span><br>
              <span v-if="selectedLoanForAction.project_name">
                Project: <span class="text-secondary">{{ selectedLoanForAction.project_name }}</span><br>
              </span>
              Tools: <span class="text-orange">{{ selectedLoanForAction.sku_items?.length || 0 }} types</span>
            </p>
